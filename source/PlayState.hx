		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, false, Paths.formatToSongPath(SONG.song) == 'cheating' && !gottaHitNote);
					...
					if(floorSus > 0) {
						for (susNote in 0...floorSus+1)
						{
							...
							var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(SONG.speed, 2)), daNoteData, oldNote, true, false, Paths.formatToSongPath(SONG.song) == 'cheating' && !gottaHitNote);
